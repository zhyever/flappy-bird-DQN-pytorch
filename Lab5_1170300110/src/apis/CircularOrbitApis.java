...
public class CircularOrbitApis<L, E> {
  ...
  /**getEntropy.
   * 计算熵 定义Σ(每条轨道上的物体数/总物体数 * ln 每条轨道上的物体数/总物体数)
   * 
   * @param c system
   * @return 熵
   */
  ...
  public Difference<E> getDifference(ConcreteCircularOrbit<L, E> c1, 
      ConcreteCircularOrbit<L, E> c2) {
    ...
    if (c1 instanceof SocialNetworkCircle) {
      ...
      for (int i = 0; i < maxTrack; i++) {
        ...
        if (arry1.size() == 0 && arry2.size() == 0) {
          ...
          d.left.put(i + 1, null);
          d.right.put(i + 1, arry2);
        } else if (arry1.size() != 0 && arry2.size() == 0) {
          d.differentNum.put(i + 1, arry1.size());
          d.left.put(i + 1, arry1);
          d.right.put(i + 1, null);
        } else {
